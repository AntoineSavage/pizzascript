...
parseSpec = describe "parse" $ do
    ...
    it "parses doubles with decimal part" $ do
        property $ \intPart (Positive decPart) -> do
            ...
            parse parser "tests" (show d) `shouldBe` Right (PzDouble d)
    ...
    it "parses doubles with exponential part" $ do
        property $ \intPart (Positive decPart) expPart -> do
            ...
            parse parser "tests" (show d1) `shouldBe` Right (fromIntOrDouble PzInteger PzDouble d1)
            parse parser "tests" (show d2) `shouldBe` Right (fromIntOrDouble PzInteger PzDouble d2)

    it "parses doubles" $ do
        property $ \d -> do
            parse parser "tests" (show $ d + 0.1) `shouldBe` Right (PzDouble $ d + 0.1)

unparseSpec :: Spec
unparseSpec = describe "unparse" $ do
    it "returns show<int> for any int" $ do
        property $ \n -> do
            unparse (PzInteger n) `shouldBe` show n

    it "returns show<int> for any int-valued double" $ do
        property $ \n -> do
            unparse (PzDouble $ fromIntegral n) `shouldBe` show (n :: Integer)

    it "returns show<double> for any non-int-valued double" $ do
        property $ \d -> do
            unparse (PzDouble $ d + 0.1) `shouldBe` show (d + 0.1)
