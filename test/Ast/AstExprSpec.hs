...
parseSpec = describe "parse" $ do
    it "parses num" $ do
        property $ \n -> do
            parse (parser ignore) "tests" (AstNum.unparse n) `shouldBe` Right (AstNum n)

    it "parses str" $ do
        property $ \s -> do
            parse (parser ignore) "tests" (AstStr.unparse s) `shouldBe` Right (AstStr s)

    it "parses ident" $ do
        property $ \i -> do
            parse (parser ignore) "tests" (AstIdent.unparse i) `shouldBe` Right (AstIdent i)

    it "parses symb" $ do
        property $ \s -> do
            parse (parser ignore) "tests" (AstSymb.unparse s) `shouldBe` Right (AstSymb s)

    it "parses list" $ do
        property $ \l -> do
            parse (parser ignore) "tests" (AstList.unparse sep (unparse sep) l) `shouldBe` Right (AstList l)

unparseSpec :: Spec
unparseSpec = describe "unparse" $ do
    it "unparses num" $ do
        property $ \n -> do
            unparse sep (AstNum n) `shouldBe` AstNum.unparse n

    it "unparses str" $ do
        property $ \s -> do
            unparse sep (AstStr s) `shouldBe` AstStr.unparse s

    it "unparses ident" $ do
        property $ \i -> do
            unparse sep (AstIdent i) `shouldBe` AstIdent.unparse i

    it "unparses symb" $ do
        property $ \s -> do
            unparse sep (AstSymb s) `shouldBe` AstSymb.unparse s
