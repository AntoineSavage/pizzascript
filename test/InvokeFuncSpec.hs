...
invokeFuncResultShowSpec = describe "InvokeFuncResult (show instance)" $ do
    it "shows ResultEvaled" $ do
        property $ \v -> v /= PzUnit ==> do
            show (ResultEvaled PzUnit) `shouldBe` "ResultEvaled PzUnit"
            show (ResultEvaled v) `shouldBe` "ResultEvaled (" ++ show v ++ ")"

    it "shows ResultPushBlock" $ do
        property $ \(ArbDict d) (Few vs) -> do
            show (ResultPushBlock (d, vs)) `shouldBe` "ResultPushBlock " ++ show (d, vs)
    ...
    it "handles built-in func (quoted)" $ do
        property $ \(ArbDict d) impArgs args e es -> do
            ...
            invokeFunc d u (Func u u $ BodyBuiltIn $ symb "func") vs `shouldBe`
                Right (ResultEvaled $ PzList [PzDict d, PzFunc d $ Func impArgs args $ BodyCustom e es])

    it "handles built-in func (evaled)" $ do
        property $ \v -> do
            invokeFunc u u (Func u u $ BodyBuiltIn $ symb "type_of") [v] `shouldBe` Right (ResultEvaled $ _typeOf v)
    ...
    it "handles custom func (quoted)" $ do
        property $ \(ArbDict ctx) (ArbDict implCtx) impArgs args e es v (Few vs'') -> do
            ...
            invokeFunc ctx implCtx (Func impArgs args $ BodyCustom e es) vs `shouldBe`
                Right (ResultPushBlock (M.union argImplCtx implCtx, e:es))
